...
class CharacterDroppingComponent(Component):
    ...
    def getItemByDropping(self, dropConfigId):
        ...
        if not dropConfig or len(dropConfig.items()) == 0:
            print 'no item for the dropconfig'
            return None
        dropcoefficient = self._dropCoefficient
        characterId = self._owner.baseInfo.id
        itemTemplate = self.getDropItemTemplate(dropConfig)
        if itemTemplate:
            sExAttrId, eExAttrId = self.getDropItemExtraAttribute(dropConfig, itemTemplate, dropcoefficient)
            list = [0, characterId, itemTemplate['id'], sExAttrId, eExAttrId, 0, 1]
            dbaccess.insertItemRecord(list)
            return dbaccess.getLastInsertItemInfo(), itemTemplate
        else:
            return None, None
    ...
    def getDropConfigOnNpc(self, npc):
        ...
        while i < len(levelgroup):
            ...
            i = i + 1
        monster_group_id = npc['monsterGroupId']
        monster_level = 1
